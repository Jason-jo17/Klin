...
import { McpResultCard } from "../components/McpResultCards";
...
const PREVIEW_PROMPTS = ["List my open issues.", "What's the deployment status of the bench service?", "Show issues assigned to morgan."];
  ...
  return (
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "var(--kiln-space-8)" }}>
      <header style={{ marginBottom: "var(--kiln-space-8)" }}>
        <h1 style={{ fontSize: "1.75rem" }}>Static GenUI — MCP Tool Cards</h1>
        <p style={{ color: "var(--kiln-text-secondary)" }}>
          Tool results come from a real MCP server (<code>lib/mcpServer.ts</code>) over an in-memory transport,
          then render into pre-built cards — see <code>components/McpResultCards.tsx</code>.
        </p>
          ...
          <div key={message.id}>
            {message.parts.map((part, i) => {
              ...
              if (part.type.startsWith("tool-") && "state" in part && part.state === "output-available") {
                return <McpResultCard key={i} output={(part as { output: { kind: string } & Record<string, unknown> }).output} />;
              }
              return null;
            })}
          </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); submit(input); }} style={{ display: "flex", gap: "var(--kiln-space-3)", marginTop: "var(--kiln-space-6)" }}>
        <div style={{ flex: 1 }}>
          <Input data-testid="chat-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about issues or deployments…" aria-label="Message" />
        </div>
